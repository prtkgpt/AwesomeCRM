...
/**
 * Format date for display in user's device timezone
 * Dates are shown in the user's local timezone for convenience
 * @param date - Date object or ISO string
 * @param formatStr - date-fns format string (default: 'MMM d, yyyy')
 * @param timezone - Optional timezone override (defaults to device timezone)
 */
export function formatDate(date: Date | string, formatStr: string = 'MMM d, yyyy', timezone?: string): string {
  ...
  // Parse the local string back to a Date object for formatting
...
/**
 * Format date and time for display in user's device timezone
 * Times are shown in the user's local timezone with timezone indicator
 * @param date - Date object or ISO string
 * @param timezone - Optional timezone override (defaults to device timezone)
 */
...
/**
 * Format time only in user's device timezone
 * Times are shown in the user's local timezone with timezone indicator
 * @param date - Date object or ISO string
 * @param timezone - Optional timezone override (defaults to device timezone)
 */
export function formatTime(date: Date | string, timezone?: string): string {
  ...
  const formatted = format(localDate, 'h:mm a');
...
/**
 * Parse a date+time string as if it's in the user's device timezone
 * Converts local time to UTC for database storage
 * @param dateStr - Date string in format "YYYY-MM-DD" or "YYYY-MM-DDTHH:mm"
 * @param timezone - Optional timezone override (defaults to device timezone)
 * @returns Date object in UTC
 * @example parseDateInLocalTZ("2026-01-20T10:00") => Date representing 10 AM local time in UTC
 */
export function parseDateInLocalTZ(dateStr: string, timezone?: string): Date {
  const tz = timezone || getDeviceTimezone();
  ...
  // Treat this date as if it's in the user's timezone and convert to UTC
...
/**
 * @deprecated Use parseDateInLocalTZ() instead
...
export function parseDateInCompanyTZ(dateStr: string): Date {
  return parseDateInLocalTZ(dateStr, 'America/Los_Angeles');
}
